...
// Importando seus dados. Se der erro, verifique se no arquivo constantes.ts o nome é 'products' mesmo.
import { products } from './constantes';

export default function App() {
  const [isCartOpen, setIsCartOpen] = useState(false);
  const [cart, setCart] = useState([]);
  const [view, setView] = useState('home'); // 'home', 'product', 'checkout', 'dashboard'
  ...
  // 3. Detalhes do Produto (Corrigido para Mobile)
  const ProductDetail = () => {
    if (!selectedProduct) return null;
    
    // Simulação de preço dinâmico (se existisse lógica complexa, estaria aqui)
    ...
    return (
      <div className="pt-24 pb-12 max-w-7xl mx-auto px-4">
        ...
        {/* CORREÇÃO MOBILE AQUI: flex-col no mobile, flex-row no desktop */}
  ...
  const Dashboard = () => (
    <div className="pt-24 pb-12 max-w-3xl mx-auto px-4">
      ...
      {!user ? (
        ...
        // Se estiver logado, mostra pedidos
        <div>
